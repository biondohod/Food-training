  ...
  elements.forEach(element => {
    element.addEventListener('click', evt => {
      if (evt.target.getAttribute('data-ratio')) {
        localStorage.setItem('ratio', +evt.target.getAttribute('data-ratio')); // ratio = +evt.target.getAttribute('data-ratio');
      } else {
        localStorage.setItem('sex', evt.target.getAttribute('id')); // sex = evt.target.getAttribute('id');
      }
...
const bindPostData = form => {
  form.addEventListener('submit', evt => {
    ...
    const formData = new FormData(form); //Form data to JSON 1 
    // const jsonObj = {};
    ...
    (0,_api_js__WEBPACK_IMPORTED_MODULE_0__.postData)('http://localhost:3000/requests', jsonObj).then(() => {
...
class MenuCard {
  constructor(src, alt, title, description, price, parentClass) {
    this.src = src;
    this.alt = alt;
    this.title = title;
...
(0,_api_js__WEBPACK_IMPORTED_MODULE_0__.getData)('http://localhost:3000/menu').then(data => {
  data.forEach(_ref => {
    let {
      img,
      altimg,
...
const createSlider = _ref => {
  ...
  try {
    ...
    sliderNext.addEventListener('click', toggleNextSlide); // Точки для слайдера
...
/* harmony import */ var _modules_menu_cards_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./modules/menu-cards.js */ "./src/js/modules/menu-cards.js");
...
/* harmony import */ var _modules_calc_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./modules/calc.js */ "./src/js/modules/calc.js");
/* harmony import */ var _modules_calc_js__WEBPACK_IMPORTED_MODULE_7___default = /*#__PURE__*/__webpack_require__.n(_modules_calc_js__WEBPACK_IMPORTED_MODULE_7__);
...
(0,_modules_slider_js__WEBPACK_IMPORTED_MODULE_6__.createSlider)({
